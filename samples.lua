...
print("var samples = {")
for id, bytecode in pairs(codes) do
  local tab = {}
  for i = 1, #id do
    tab[i] = string.format("%02x", id:byte(i))
  end
  print(string.format("  '%s': new Uint8Array([", table.concat(tab)))
  tab = {}
  for i = 1, #bytecode do
    tab[i] = string.format("0x%02x", bytecode:byte(i))
  end
